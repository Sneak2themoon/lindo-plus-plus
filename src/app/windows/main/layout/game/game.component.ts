...
import {AfterViewInit, Component, EventEmitter, Input, isDevMode, NgZone, Output} from '@angular/core';
...
export class GameComponent implements AfterViewInit {
    ...
    public setMods(): void {
        //on "charge l'api avant"
        for (let mod in Mods) {
            switch (mod) {
                case 'API':
                    this.api = new Mods.API(this.game.window, this.settingsService, this.translateService)
                break
                default:
        ...
        for (const mod in Mods) {
            switch (mod) {
                case 'Moreui':
                    let moreui = new Mods[mod](this.game.window, this.settingsService, this.translateService, this.api);
                    this.mods.push(moreui)
                    break
                case 'Controller':
                    let controller = new Mods[mod](this.game.window, this.settingsService, this.translateService, this.api);
                    this.mods.push(controller)
                    break
                case 'API':
                    break
                case 'AutoGroup':
