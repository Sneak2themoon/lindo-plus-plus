...
export class PathFinderFauxGps extends Mod{

    mapPoints
    grid
    useOldMovementSystem
    firstCellZone
    OCCUPIED_CELL_WEIGHT
    ELEVATION_TOLERANCE
    WIDTH
    HEIGHT
    CELL_NUMBER
    ...
    startMod(): void {
        ...
        Logger.info("- PathFinderFauxGps active");
    }
 ...
 * 
 */
    public getPath(source, target, occupiedCells, allowDiagonals, stopNextToTarget) {
    var c, candidate;
    ...
    var selection = this.getCellPathCandidate(si, sj, 0, distSrcDst, null);
    ...
    public updateCellPath(cell, cellPath) {
    ...
    public getCellPathCandidate = (_i, _j, _w, _d, _path) => {
        return { i: _i, j: _j, w: _w, d: _d, path: _path };
    };

    public addCandidate(c, w, di, dj, candidates, path) {
        var i = c.i;
        var j = c.j;
        ...
        if (c.candidateRef === null) {
            var candidateRef = this.getCellPathCandidate(i, j, path.w + w, distanceToDestination, path);
    ...
    public addCandidates(path, di, dj, candidates, allowDiagonals) {
        var i = path.i;
        var j = path.j;
        var c = this.grid[i][j];
    ...
    public getCellId(x, y) {
        return this.mapPoints[x + "_" + y];
    }

    public getCellPathData = (_i, _j) => {
        return { i: _i, j: _j, floor: -1, zone: -1, speed: 1, weight: 0, candidateRef: null };
    };

    public _generateMapPoints() {
        this.mapPoints = {};
    ...
    public  _generateGrid() {
        this.grid = []
        for (let i = 0; i < this.WIDTH; i += 1) {
          const row = [];
          for (let j = 0; j < this.HEIGHT; j += 1) {
            row[j] = this.getCellPathData(i, j);
          }
          this.grid[i] = row;
        }
    }
    ...
    public areCommunicating(c1, c2, oldMovementSystem = false) {
    ...
    public canMoveDiagonallyTo(c1, c2, c3, c4) {
    ...
    public reset() {
        ...
        Logger.info(' - PathFinderFauxGps deactiver')
    }
}